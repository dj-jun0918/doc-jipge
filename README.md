...
표준 7종 자격요건을 **값·연산자 완전 일치** 기준으로 엄격 채점한다. GT 53건(일반 44 + 교란 9) 기준 추출 **F1 0.633** (정밀도 0.613 / 재현율 0.655, Bootstrap 95% CI [0.518, 0.745] — 표본이 작아 과대 해석 금지), 라벨러 일치도 Cohen's **κ 0.665**(교차라벨 10건 중 6건이 동일 복사본이라 과대 추정, 독립 4건 기준 κ≈0.49·불안정), 근거 원문 verbatim 100%. 측정 결과 전체는 자동 생성되는 `evaluation/results/report.md`에서 확인할 수 있다.
